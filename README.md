...
## Merge automation for generated reports

To automatically keep the incoming PR version of generated reports during local merges, run this one-time setup:
...
```powershell
git config merge.keepIncoming.name "keep incoming version"
git config merge.keepIncoming.driver 'powershell -NoProfile -Command "Copy-Item -Force ''%B'' ''%A''"'
```

This works for local Git merges only; GitHub web conflict resolution does not use custom local merge drivers.
